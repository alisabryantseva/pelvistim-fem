...
def main():
    ...
    # Thresholds
    compliance_limit  = 100.0   # V — typical PTNS device max output voltage
    ...
    line_kw   = dict(marker="o", markersize=5, linewidth=2)
    xlabel    = "Contact conductivity  σ_c  (S/m)         loose  →  tight  →"
    leg_kw    = dict(fontsize=8, labelcolor="white",
                     facecolor="#111111", edgecolor="#444444")

    # ── Panel 1: Compliance voltage ───────────────────────────────────────────
    ax = axes[0, 0]
    ax.semilogx(x, comp_V, color="#ff6b35", **line_kw)
    ax.axhline(compliance_limit, color="#ff4444", linestyle="--", linewidth=1.5,
               label=f"Device limit  {compliance_limit:.0f} V\n"
                     r"(above → stimulator cannot deliver 5 mA)")
    # Shade "too low pressure" region
    ax.fill_between(x, compliance_limit, max(comp_V) * 1.05,
                    where=[v > compliance_limit for v in comp_V],
                    color="#ff4444", alpha=0.10)
    ...
    ax = axes[0, 1]
    ax.loglog(x, Z_ohm, color="#4ecdc4", **line_kw)
    ax.set_xlabel(xlabel)
    ax.set_ylabel("Contact impedance  Z  (Ω)")
    ax.set_title("Contact Impedance vs Wrap Pressure")
    ax.set_xticks(x_ticks)
    ax.set_xticklabels(x_labels, fontsize=8)
    ax.grid(True, color="#333333", linestyle="--", alpha=0.5, which="both")

    # ── Panel 3: Charge density ───────────────────────────────────────────────
    ax = axes[1, 0]
    ax.semilogx(x, charge, color="#ffd166", **line_kw)
    ax.axhline(charge_limit, color="#ff4444", linestyle="--", linewidth=1.5,
               label=f"Tissue damage limit  {charge_limit} mC/cm²")
    ax.set_xlabel(xlabel)
    ...
    # ── Panel 4: ROI E-field ──────────────────────────────────────────────────
    ax = axes[1, 1]
    ax.semilogx(x, roi_E, color="#a8dadc", **line_kw)
    ax.axhline(nerve_thresh_E, color="#ff4444", linestyle="--", linewidth=1.5,
               label=f"Nerve activation threshold  ~{nerve_thresh_E:.0f} V/m\n"
                     "(below → insufficient tibial nerve stimulation)")
    # Shade sub-threshold region
    ax.fill_between(x, 0, nerve_thresh_E,
                    color="#ff4444", alpha=0.10)
    ...
    meta = (f"Fixed: t_fat={d0['t_fat_mm']:.0f} mm   r={d0['elec_r_mm']:.0f} mm   "
            ...
            f"pw={d0['pulse_width_us']:.0f} µs   |   {date.today()}")
    fig.text(0.5, 0.005, meta, ha="center", va="bottom",
             fontsize=8, color="#888888")

    fig.suptitle("Step 04 — Pressure-Dependent Contact Impedance Sweep  (15 levels, p01–p15)",
